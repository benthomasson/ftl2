"""Edge-case tests for become_prefix — supplements tests/test_become.py."""

import pytest
import sys
sys.path.insert(0, "/Users/ben/git/beliefs-pi/workspaces/issue-4/src")
...
class TestBecomeEdgeCases:
    ...
    def test_su_single_quote_in_command(self):
        """Reviewer concern: su -c uses single quotes — what if cmd has them?"""
        bc = BecomeConfig(become=True, become_method="su")
        # This will produce broken shell: su - root -c 'echo 'hello''
        # Documenting the behavior — it wraps literally without escaping
    ...
    def test_su_root_explicit(self):
        """Plan success criterion: su with default root user."""
        bc = BecomeConfig(become=True, become_method="su")
